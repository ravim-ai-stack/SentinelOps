...
@router.get("/api/jobs/failures-by-cause")
def failures_by_cause(request: Request):
    runs = fetch_job_runs(request, WINDOW_DAYS)

    failed = [
        r
        for r in runs
        if bucket_result_state(r["result_state"]) == "failed"
    ]

    counts = Counter(
        cause_label(r.get("termination_code"))
        for r in failed
    )

    causes = [
        {"cause": cause, "count": n}
        for cause, n in counts.most_common(5)
    ]
