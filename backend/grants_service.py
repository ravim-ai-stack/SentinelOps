...
@ttl_cache(CACHE_SECONDS)
def fetch_scim_users() -> list[dict]:
    """Read SCIM users from pre-aggregated table (SP, not viewer token).
    Returns same shape as SCIM REST API: list of dicts with id, userName,
    displayName, active."""
    return run_query(
        """
...
@ttl_cache(CACHE_SECONDS)
def fetch_scim_groups() -> list[dict]:
    """Read SCIM groups from pre-aggregated table (SP, not viewer token).
    Returns same shape as SCIM REST API: list of dicts with id, displayName,
